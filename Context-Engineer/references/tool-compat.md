...
|---------|-------------|--------|----------|----------|---------|
| Root config | `AI.md` | `.cursorrules` | `.windsurfrules` | `AI.md` | `AI.md` |
| Directory indexes | `AI.md` | `AI.md` | `AI.md` | `AI.md` | `AI.md` |
...
**Files to generate:**
- `AI.md` at repo root (auto-loaded into every conversation)
- `AI.md` in each directory (loaded when AI navigates there)

**Root AI.md considerations:**
- Keep under 200 lines — everything beyond ~200 lines risks being truncated
- Put the most critical information first
- Use sub-directory AI.md files for detailed content
- Link to sub-directory indexes: `[context/](context/AI.md)`
...
Cursor reads `.cursorrules` at the project root. It doesn't natively read `AI.md`, but Cursor's AI can still read any file when asked.

**Files to generate:**
- `.cursorrules` at repo root — this is auto-loaded
- `AI.md` in each directory — not auto-loaded, but AI can read them when exploring

**`.cursorrules` format:**
Same content as `AI.md`, but note:
...
- Tell the AI explicitly: "When working in a directory, read its AI.md for detailed context"
...
When working in any directory, check for a AI.md file for detailed context about that directory's contents.
...
- `AI.md` in each directory (readable but not auto-loaded)

Same format and length considerations as `.cursorrules`.

### OpenClaw

OpenClaw is evolving rapidly. As of early 2026, it reads `AI.md` by default but behavior may vary by version.

**Files to generate:**
- `AI.md` at repo root
- `AI.md` in each directory

Treat it like Claude Code for now. If the user reports issues, adjust.

### Generic / Unknown Tool

Default to `AI.md` everywhere. It's the most portable format — any AI tool can read markdown files even if it doesn't auto-load them.
...
1. Generate `AI.md` files everywhere (works for both)
2. Also generate `.cursorrules` at root (for Cursor users)
3. Keep both files in sync — the root `.cursorrules` should be a condensed version of the root `AI.md`

**Important:** Don't duplicate content across config files. The root tool-specific file (`.cursorrules`, `.windsurfrules`) should be a concise pointer that tells the AI where to find detailed context. The `AI.md` files contain the actual content.
